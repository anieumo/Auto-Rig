...
window = pm.window(title="Test Window")
...
def makeOval():
    ...
    #pm.curve(n='oval1', d=3, p=[(0, 0, -1), (2, 0, -1), (3, 0, 0), (2, 0, 1), (0, 0, 1)])
    #pm.attachCurve('oval', 'oval1', rpo=True, n='oval')
    

def makeArrow():
    pm.curve(n='arrow', d=1, p=[(0, 0, -1), (1, 0, -1), (1, 0, -2), (3, 0, 0), (1, 0, 2), (1, 0, 1), (0, 0, 1)])
    # pm.curve(n='arrow', d=1, p=[(0, 0, -1), (length, 0, -1), (length, 0, -2), (length + 2, 0, 0), (1length, 0, 2), (length, 0, 1), (0, 0, 1)]
...
def symmetricizetoright():
    ...
    for item in deletelist:
        if 'leg1' in item:
            selection = pm.listRelatives(item, ad=True)
    ...
    for item in joints:
        a = pm.getAttr(item + '.translateX')
        b = pm.getAttr(item + '.translateY')
        c = pm.getAttr(item + '.translateZ')
        if 'left_leg1' in item.name():
            print(item) 
            #pm.duplicate()
            #pm.move(-a, b, c)
            pm.mirrorJoint(item, mirrorBehavior= True, myz=True, searchReplace=('right_', 'left_'))
       
        if 'left_arm1' in item.name():
            #pm.duplicate(item, n='right_' + item)
            #print(a, b, c)
            #pm.move(-a, 0, c)
            pm.mirrorJoint(item, mirrorBehavior= True, myz=True, searchReplace=('right_', 'left_'))
    ...
    for item in joints:
        print(item)
        if 'arm_11' in item.name():
            print(item)
            pm.delete(item)
        if 'arm_10' in item.name():
            pm.delete(item)
         
def symmetricizetoleft():
    ...
    for item in joints:
        ...
        if 'left_leg1' in item.name():
            ...
            #pm.duplicate()
            #pm.move(-a, b, c)
            pm.mirrorJoint(item, mirrorBehavior= True, myz=True, searchReplace=('left_', 'right_'))
...
def parentjoints():
    ...
    for item in joints:
        #pm.makeIdentity(item, apply=True)
        ...
        #pm.makeIdentity(contrl, apply=True)
        #contrlcurve = pm.listRelatives(contrl)[0]
        #orient constrain here and point constraint the IK 
        #pm.parent(contrl, item, s=True, r=False)
        #pm.parentConstraint(contrl, item, mo=True)
...
def IKhookup():
    #LEG
    #duplicating and renaming duplicates
    '''
    pm.select('left_leg1', replace=True)
    pm.duplicate()
    pm.rename('left_leg4', 'IK_left_leg1')
    pm.duplicate()
    pm.rename('IK_left_leg2', 'FK_left_leg1')
    
    leg1 = pm.listRelatives('IK_left_leg1', c=True, ad=True)
    leg2 = pm.listRelatives('FK_left_leg1', c=True, ad=True)
    for item in leg1:
        newname = item.name().replace("left", "IK_left")
    ...
    pm.duplicate()
    pm.rename('right_leg4', 'IK_right_leg1')
    pm.duplicate()
    pm.rename('IK_right_leg2', 'FK_right_leg1')
    leg1 = pm.listRelatives('IK_right_leg1', c=True, ad=True)
    leg2 = pm.listRelatives('FK_right_leg1', c=True, ad=True)
    for item in leg1:
        newname = item.name().replace("right", "IK_right")
        pm.rename(item.name(), newname)
    for item in leg2:
    ...
    print('c')
   #orient contraint is not working
    ...
    #can do point constraint
    pm.pointConstraint('IK_right_leg1', 'FK_right_leg1', 'right_leg1', w=1)
    pm.pointConstraint('IK_right_leg2', 'FK_right_leg2', 'right_leg2', w=1)
    pm.pointConstraint('IK_right_leg3', 'FK_right_leg3', 'right_leg3', w=1)
    ...
    #ARM
    #switch left arm to FK_left_arm
    ...
    '''
    pm.setAttr('FK_right_arm1.visibility', 0)
    pm.setAttr('FK_left_arm1.visibility', 0)
    pm.setAttr('IK_right_arm1.visibility', 0)
    pm.setAttr('IK_left_arm1.visibility', 0)
    '''
...
button = pm.button(label="Step4: symmetricize to the right-->", command='symmetricizetoright()', w=10)
button = pm.button(label="Step4: symmetricize to the left<--", command='symmetricizetoleft()', w=10)
